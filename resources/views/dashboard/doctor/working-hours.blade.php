            ...
            @php
                ...
                foreach ($days as $day) {
                    if (!isset($workingHours[$day])) {
                        $workingHours[$day] = ['start' => '', 'end' => '', 'types' => []];
                    }
                }
            @endphp
            ...
            <tbody>
                @foreach($workingHours as $weekday => $hours)
                    <tr>
                        ...
                        <td><input type="time" name="working_hours[{{ $weekday }}][start]" class="form-control"
                                value="{{ $hours['start'] }}"></td>
                        <td><input type="time" name="working_hours[{{ $weekday }}][end]" class="form-control"
                                value="{{ $hours['end'] }}"></td>
                        <td>
                            @foreach($appointmentTypes as $type)
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="checkbox" name="working_hours[{{ $weekday }}][types][]"
                                        value="{{ $type->id }}" {{ in_array($type->id, $hours['types'] ?? []) ? 'checked' : '' }}>
